...
public class SessionFactoryImpl implements SessionFactory
{
	...
	{
		return new S13ClientSessionImpl(request.getSessionId(), request.getDestinationHost(), request.getDestinationRealm(), provider);
	}

	@Override
	public S13ServerSession createServerSession(MEIdentityCheckRequest request) throws AvpNotSupportedException
	{
		return new S13ServerSessionImpl(request.getSessionId(), request.getOriginHost(), request.getOriginRealm(), provider);
	}
}