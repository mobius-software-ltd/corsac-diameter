...
public class GCSProviderImpl extends DiameterProviderImpl<ClientListener, ServerListener, AvpFactory, MessageFactory, SessionFactory>
{
	public GCSProviderImpl(DiameterStack stack)
	{
	...
	{		
		try
		{
			if(message instanceof GCSActionRequest)
				return new GCSActionServerSessionImpl(message.getSessionId(), message.getOriginHost(), message.getOriginRealm(), this);
			else if(message instanceof GCSNotificationRequest)
			return new GCSNotificationServerSessionImpl(message.getSessionId(), message.getOriginHost(), message.getOriginRealm(), this);
		}
