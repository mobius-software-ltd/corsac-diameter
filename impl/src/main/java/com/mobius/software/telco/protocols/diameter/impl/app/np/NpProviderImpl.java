...
public class NpProviderImpl extends DiameterProviderImpl<ClientListener, ServerListener, AvpFactory, MessageFactory, SessionFactory>
{
	public NpProviderImpl(DiameterStack stack)
	{
	...
	{		
		try
		{
			if(message instanceof AggregatedRUCIReportRequest)
				return new AggregatedRUCIReportServerSessionImpl(message.getSessionId(), message.getOriginHost(), message.getOriginRealm(), this);
			else if(message instanceof ModifyUeContextRequest)
				return new ModifyUeContextServerSessionImpl(message.getSessionId(), message.getOriginHost(), message.getOriginRealm(), this);
			else if(message instanceof NonAggregatedRUCIReportRequest)
				return new NonAggregatedRUCIReportServerSessionImpl(message.getSessionId(), message.getOriginHost(), message.getOriginRealm(), this);
