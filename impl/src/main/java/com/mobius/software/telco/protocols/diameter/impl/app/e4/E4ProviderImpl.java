...
{
	public E4ProviderImpl(DiameterStack stack)
	{
	...
	{		
		try
		{
			if(message instanceof PushNotificationRequest)
				return new E4PushNotificationServerSessionImpl(message.getSessionId(), message.getOriginHost(), message.getOriginRealm(), this);
			else if(message instanceof UserDataRequest)
				return new E4UserDataServerSessionImpl(message.getSessionId(), message.getOriginHost(), message.getOriginRealm(), this);
		}
