...
*
*/
public class S9ProviderImpl extends DiameterProviderImpl<ClientListener, ServerListener, AvpFactory, MessageFactory, SessionFactory>
{
	public static Logger logger=LogManager.getLogger(S9ProviderImpl.class);
	...
	{		
		try
		{
			if(message instanceof CreditControlRequest)
				return new S9ServerSessionImpl(true, message.getSessionId(), message.getOriginHost(), message.getOriginRealm(), this);
			else if(message instanceof TriggerEstablishmentRequest)
				return new S9ServerSessionImpl(false, message.getSessionId(), message.getOriginHost(), message.getOriginRealm(), this);
		}
