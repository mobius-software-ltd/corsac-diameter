...
import com.mobius.software.telco.protocols.diameter.app.rfc5778a.Rfc5778aClientSession;
...
public class Rfc5778aClientSessionImpl implements Rfc5778aClientSession
{
	...
	public Rfc5778aClientSessionImpl(Boolean isAuth, String sessionID, String remoteHost, String remoteRealm, DiameterProvider<? extends ClientListener, ?, ?, ?, ?> provider)
	{
		if(isAuth==null || isAuth)
			authSession = new ClientAuthSessionImpl<MIP6Request, MIP6Answer,ReAuthRequest,ReAuthAnswer,AbortSessionRequest,AbortSessionAnswer,SessionTerminationRequest,SessionTerminationAnswer>(sessionID, Long.valueOf(ApplicationIDs.MIP6A), remoteHost, remoteRealm, provider);
		else
			accSession = new ClientAccSessionImpl<AccountingRequest, AccountingAnswer>(sessionID, Long.valueOf(ApplicationIDs.MIP6A), remoteHost, remoteRealm, provider);		
	}
	...
	@Override
	public void sendInitialRequest(MIP6Request request, AsyncCallback callback)
	{
		if(authSession != null)
