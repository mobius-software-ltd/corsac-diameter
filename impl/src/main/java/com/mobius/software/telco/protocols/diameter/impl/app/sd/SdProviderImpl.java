	...
	{		
		try
		{
			if(message instanceof CreditControlRequest)
				return new SdServerSessionImpl(true, message.getSessionId(), message.getOriginHost(), message.getOriginRealm(), this);
			else if(message instanceof TDFSessionRequest)
				return new SdServerSessionImpl(false, message.getSessionId(), message.getOriginHost(), message.getOriginRealm(), this);
		}
