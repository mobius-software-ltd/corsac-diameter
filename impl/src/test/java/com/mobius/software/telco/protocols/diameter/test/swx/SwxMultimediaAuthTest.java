...
*
*/
public class SwxMultimediaAuthTest extends NetworkTestBase
{
	protected static final String localListenerID = "1";
	private static final Logger logger = LogManager.getLogger(SwxMultimediaAuthTest.class);
	
	@Test
	public void testSwxMultimediaAuth() throws Exception
	{
		super.setupRemote(0L,0L);
		super.setupLocal(0L,0L);
		
		final AtomicLong maaReceived=new AtomicLong(0L);
		final AtomicLong maaReceivedByListener=new AtomicLong(0L);
		final AtomicLong marReceived=new AtomicLong(0L);
		final AtomicLong marReceivedByListener=new AtomicLong(0L);
		...
		provider.setClientListener(listenerID, new ClientListener()
		{
			...
			{
				maaReceivedByListener.incrementAndGet();
			}
		});
		...
		serverProvider.setServerListener(listenerID, new ServerListener()
		{
			...
			{
				String username=new String("q");
				marReceivedByListener.incrementAndGet();
				MultimediaAuthRequest mar=(MultimediaAuthRequest)request;
				try
				{
					MultimediaAuthAnswer maa = serverProvider.getMessageFactory().createMultimediaAuthAnswer(mar, mar.getHopByHopIdentifier(), mar.getEndToEndIdentifier(), ResultCodes.DIAMETER_SUCCESS,username);
					session.sendInitialAnswer(maa, new AsyncCallback()
					{
						@Override
						...
						{
							logger.error("An error occured while sending Multimedia Auth Answer," + ex.getMessage(),ex);
						}
					});
				}
		...
		String username=new String("q");
		Long sipNumberAuthItems=new Long(12345);
		SIPAuthDataItem sipAuthDataItem = provider.getAvpFactory().getSIPAuthDataItem();
		
		MultimediaAuthRequest request = provider.getMessageFactory().createMultimediaAuthRequest(localLink.getLocalHost(), localLink.getLocalRealm(), localLink.getDestinationHost(), localLink.getDestinationRealm(),username,sipNumberAuthItems,sipAuthDataItem);
		...
		clientSession.sendInitialRequest(request, new AsyncCallback()
		{
			...
			public void onError(DiameterException ex)
			{
				logger.error("An error occured while sending Multimedia Auth Request," + ex.getMessage(),ex);
			}
		});
		...
		assertEquals(maaReceived.get() , 1L);
		assertEquals(maaReceivedByListener.get() , 1L);
		assertEquals(marReceived.get() , 1L);
		assertEquals(marReceivedByListener.get() , 1L);
	...
	public void testDuplicate() throws Exception
	{
		...
		localStack.getNetworkManager().addNetworkListener(localListenerID, new NetworkListener()
		{
			...
			public void onMessage(DiameterMessage message, String linkID, AsyncCallback callback)
			{
				if(message instanceof MultimediaAuthAnswer)
					marReceived.incrementAndGet();				
			}
		});
		...
		serverStack.getNetworkManager().addNetworkListener(localListenerID, new NetworkListener()
		{
			...
			public void onMessage(DiameterMessage message, String linkID, AsyncCallback callback)
			{
				if(message instanceof MultimediaAuthRequest)
					maaReceived.incrementAndGet();				
			}
		});
		...
		assertEquals(maaReceived.get() , 1L);
		assertEquals(maaReceivedByListener.get() , 1L);
		assertEquals(marReceived.get() , 2L);
		assertEquals(marReceivedByListener.get() , 1L);
		assertEquals(timeoutReceived.get() , 0L);
	}		
}