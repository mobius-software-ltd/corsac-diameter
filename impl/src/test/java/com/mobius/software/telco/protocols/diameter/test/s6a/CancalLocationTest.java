...
*
*/
public class CancalLocationTest extends NetworkTestBase
{
	protected static final String localListenerID = "1";
	private static final Logger logger = LogManager.getLogger(CancalLocationTest.class);
	
	@Test
	public void testCancalLocation() throws Exception
	{
		super.setupRemote(0L,0L);
		super.setupLocal(0L,0L);
		
		final AtomicLong claReceived=new AtomicLong(0L);
		final AtomicLong claReceivedByListener=new AtomicLong(0L);
		final AtomicLong clrReceived=new AtomicLong(0L);
		final AtomicLong clrReceivedByListener=new AtomicLong(0L);
		...
		provider.setClientListener(listenerID, new ClientListener()
		{
			...
			{
				claReceivedByListener.incrementAndGet();
			}
		...
		serverProvider.setServerListener(listenerID, new ServerListener()
		{
			...
			{
				clrReceivedByListener.incrementAndGet();
				CancelLocationRequest clr=(CancelLocationRequest)request;
				try
				{
					CancelLocationAnswer cla = serverProvider.getMessageFactory().createCancelLocationAnswer(clr, clr.getHopByHopIdentifier(), clr.getEndToEndIdentifier(), ResultCodes.DIAMETER_SUCCESS);
					session.sendInitialAnswer(cla, new AsyncCallback()
					{
						@Override
						...
						{
							logger.error("An error occured while sending Cancel Location Answer," + ex.getMessage(),ex);
						}
					});
				}
		...
		CancelLocationRequest request = provider.getMessageFactory().createCancelLocationRequest(localLink.getLocalHost(), localLink.getLocalRealm(), localLink.getDestinationHost(), localLink.getDestinationRealm(),CancellationTypeEnum.INITIAL_ATTACH_PROCEDURE);
		...
		assertEquals(claReceived.get() , 1L);
		assertEquals(claReceivedByListener.get() , 1L);
		assertEquals(clrReceived.get() , 1L);
		assertEquals(clrReceivedByListener.get() , 1L);
