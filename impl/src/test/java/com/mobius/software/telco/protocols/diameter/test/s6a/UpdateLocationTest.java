...
*
*/
public class UpdateLocationTest extends NetworkTestBase
{
	protected static final String localListenerID = "1";
	private static final Logger logger = LogManager.getLogger(UpdateLocationTest.class);
	...
	public void testAuthenticationInformation() throws Exception
	{
		...
		final AtomicLong ulaReceived=new AtomicLong(0L);
		final AtomicLong ulaReceivedByListener=new AtomicLong(0L);
		final AtomicLong ulrReceived=new AtomicLong(0L);
		final AtomicLong ulrReceivedByListener=new AtomicLong(0L);
		...
		provider.setClientListener(listenerID, new ClientListener()
		{
			...
			{
				ulaReceivedByListener.incrementAndGet();
			}
		...
		serverProvider.setServerListener(listenerID, new ServerListener()
		{
			...
			{
				ulrReceivedByListener.incrementAndGet();
				UpdateLocationRequest ulr=(UpdateLocationRequest)request;
				try
				{
					UpdateLocationAnswer aia = serverProvider.getMessageFactory().createUpdateLocationAnswer(ulr, ulr.getHopByHopIdentifier(), ulr.getEndToEndIdentifier(), ResultCodes.DIAMETER_SUCCESS);
					session.sendInitialAnswer(aia, new AsyncCallback()
					{
						...
						{
							logger.error("An error occured while sending Update Location Answer," + ex.getMessage(),ex);
						}
					});
				}
		...
		ULRFlags ulrFlags = provider.getAvpFactory().getULRFlags();
		ulrFlags.setDualRegistration5GIndicationBit(true);;
		
		
		UpdateLocationRequest request = provider.getMessageFactory().createUpdateLocationRequest(localLink.getLocalHost(), localLink.getLocalRealm(), localLink.getDestinationHost(), localLink.getDestinationRealm(),RATTypeEnum.CDMA2000_1X,ulrFlags,visitedPLMNId);
		...
		assertEquals(ulaReceived.get() , 1L);
		assertEquals(ulaReceivedByListener.get() , 1L);
		assertEquals(ulrReceived.get() , 1L);
		assertEquals(ulrReceivedByListener.get() , 1L);
