...
*
*/
public class ResetTest extends NetworkTestBase
{
	protected static final String localListenerID = "1";
	private static final Logger logger = LogManager.getLogger(ResetTest.class);
	...
	public void testReset() throws Exception
	{
		...
		final AtomicLong rsaReceived=new AtomicLong(0L);
		final AtomicLong rsaReceivedByListener=new AtomicLong(0L);
		final AtomicLong rsrReceived=new AtomicLong(0L);
		final AtomicLong rsrReceivedByListener=new AtomicLong(0L);
		...
		provider.setClientListener(listenerID, new ClientListener()
		{
			...
			{
				rsaReceivedByListener.incrementAndGet();
			}
		...
		serverProvider.setServerListener(listenerID, new ServerListener()
		{
			...
			{
				rsrReceivedByListener.incrementAndGet();
				ResetRequest rsr=(ResetRequest)request;
				try
				{
					ResetAnswer rsa = serverProvider.getMessageFactory().createResetAnswer(rsr, rsr.getHopByHopIdentifier(), rsr.getEndToEndIdentifier(), ResultCodes.DIAMETER_SUCCESS);
					session.sendInitialAnswer(rsa, new AsyncCallback()
					{
						@Override
						...
						{
							logger.error("An error occured while sending Reset Answer," + ex.getMessage(),ex);
						}
					});
				}
		...
		ResetRequest request = provider.getMessageFactory().createResetRequest(localLink.getLocalHost(), localLink.getLocalRealm(), localLink.getDestinationHost(), localLink.getDestinationRealm());
		...
		assertEquals(rsaReceived.get() , 1L);
		assertEquals(rsaReceivedByListener.get() , 1L);
		assertEquals(rsrReceived.get() , 1L);
		assertEquals(rsrReceivedByListener.get() , 1L);
