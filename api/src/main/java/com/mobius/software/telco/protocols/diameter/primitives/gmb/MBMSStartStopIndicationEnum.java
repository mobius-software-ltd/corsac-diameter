 ...
 */
public enum MBMSStartStopIndicationEnum implements IntegerEnum
{
	START(0),STOP(1),UPDATE(2),HEARTBEAT(3);

	private static final Map<Integer, MBMSStartStopIndicationEnum> intToTypeMap = new HashMap<Integer, MBMSStartStopIndicationEnum>();
	static
	{
	    for (MBMSStartStopIndicationEnum type : MBMSStartStopIndicationEnum.values()) 
	    {
	    	intToTypeMap.put(type.value, type);
	    }
	}

	public static MBMSStartStopIndicationEnum fromInt(Integer value) 
	{
		MBMSStartStopIndicationEnum type = intToTypeMap.get(value);
	    return type;
	}
	
	private int value;
	
	private MBMSStartStopIndicationEnum(int value)
	{
		this.value=value;
	}
