 ...
 */
public enum MDTUserConsentEnum implements IntegerEnum
{
	CONSENT_NOT_GIVEN(0),CONSENT_GIVEN(1);

	private static final Map<Integer, MDTUserConsentEnum> intToTypeMap = new HashMap<Integer, MDTUserConsentEnum>();
	static
	{
	    for (MDTUserConsentEnum type : MDTUserConsentEnum.values()) 
	    {
	    	intToTypeMap.put(type.value, type);
	    }
	}

	public static MDTUserConsentEnum fromInt(Integer value) 
	{
		MDTUserConsentEnum type = intToTypeMap.get(value);
	    return type;
	}
	
	private int value;
	
	private MDTUserConsentEnum(int value)
	{
		this.value=value;
	}
