 ...
 */
public enum ChargingCorrelationIndicatorEnum implements IntegerEnum
{
	CHARGING_IDENTIFIER_REQUIRED(0);

	private static final Map<Integer, ChargingCorrelationIndicatorEnum> intToTypeMap = new HashMap<Integer, ChargingCorrelationIndicatorEnum>();
	static
	{
	    for (ChargingCorrelationIndicatorEnum type : ChargingCorrelationIndicatorEnum.values()) 
	    {
	    	intToTypeMap.put(type.value, type);
	    }
	}

	public static ChargingCorrelationIndicatorEnum fromInt(Integer value) 
	{
		ChargingCorrelationIndicatorEnum type = intToTypeMap.get(value);
	    return type;
	}
	
	private int value;
	
	private ChargingCorrelationIndicatorEnum(int value)
	{
		this.value=value;
	}
