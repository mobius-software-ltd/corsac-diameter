 ...
 */
public enum BearerControlModeEnum implements IntegerEnum
{
	UE_ONLY(0),RESERVED(1),UE_NW(2);

	private static final Map<Integer, BearerControlModeEnum> intToTypeMap = new HashMap<Integer, BearerControlModeEnum>();
	static
	{
	    for (BearerControlModeEnum type : BearerControlModeEnum.values()) 
	    {
	    	intToTypeMap.put(type.value, type);
	    }
	}

	public static BearerControlModeEnum fromInt(Integer value) 
	{
		BearerControlModeEnum type = intToTypeMap.get(value);
	    return type;
	}
	
	private int value;
	
	private BearerControlModeEnum(int value)
	{
		this.value=value;
	}
