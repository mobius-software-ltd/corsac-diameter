 ...
 */
public enum SessionReleaseCauseEnum implements IntegerEnum
{
	UNSPECIFIED_REASON(0),UE_SUBSCRIPTION_REASON(1),INSUFFICIENT_SERVER_RESOURCES(2),IP_CAN_SESSION_TERMINATION(3),UE_IP_ADDRESS_RELEASE(4); 

	private static final Map<Integer, SessionReleaseCauseEnum> intToTypeMap = new HashMap<Integer, SessionReleaseCauseEnum>();
	static
	{
	    for (SessionReleaseCauseEnum type : SessionReleaseCauseEnum.values()) 
	    {
	    	intToTypeMap.put(type.value, type);
	    }
	}

	public static SessionReleaseCauseEnum fromInt(Integer value) 
	{
		SessionReleaseCauseEnum type = intToTypeMap.get(value);
	    return type;
	}
	
	private int value;
	
	private SessionReleaseCauseEnum(int value)
	{
		this.value=value;
	}
