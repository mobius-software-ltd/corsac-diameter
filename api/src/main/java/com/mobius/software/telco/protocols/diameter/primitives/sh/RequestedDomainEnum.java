 ...
 */
public enum RequestedDomainEnum implements IntegerEnum
{
	CS_DOMAIN(0),PS_DOMAIN(1);

	private static final Map<Integer, RequestedDomainEnum> intToTypeMap = new HashMap<Integer, RequestedDomainEnum>();
	static
	{
	    for (RequestedDomainEnum type : RequestedDomainEnum.values()) 
	    {
	    	intToTypeMap.put(type.value, type);
	    }
	}

	public static RequestedDomainEnum fromInt(Integer value) 
	{
		RequestedDomainEnum type = intToTypeMap.get(value);
	    return type;
	}
	
	private int value;
	
	private RequestedDomainEnum(int value)
	{
		this.value=value;
	}
