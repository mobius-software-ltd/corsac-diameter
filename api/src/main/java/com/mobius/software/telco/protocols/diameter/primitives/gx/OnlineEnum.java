 ...
 * 
 */
public enum OnlineEnum implements IntegerEnum
{
	DISABLE_ONLINE(0),ENABLE_ONLINE(1);

	private static final Map<Integer, OnlineEnum> intToTypeMap = new HashMap<Integer, OnlineEnum>();
	static
	{
	    for (OnlineEnum type : OnlineEnum.values()) 
	    {
	    	intToTypeMap.put(type.value, type);
	    }
	}

	public static OnlineEnum fromInt(Integer value) 
	{
		OnlineEnum type = intToTypeMap.get(value);
	    return type;
	}
	
	private int value;
	
	private OnlineEnum(int value)
	{
		this.value=value;
	}
