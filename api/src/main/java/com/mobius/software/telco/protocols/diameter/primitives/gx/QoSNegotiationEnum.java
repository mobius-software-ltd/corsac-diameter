 ...
 */
public enum QoSNegotiationEnum implements IntegerEnum
{
	NO_QOS_NEGOTIATION(0),QOS_NEGOTIATION_SUPPORTED(1);

	private static final Map<Integer, QoSNegotiationEnum> intToTypeMap = new HashMap<Integer, QoSNegotiationEnum>();
	static
	{
	    for (QoSNegotiationEnum type : QoSNegotiationEnum.values()) 
	    {
	    	intToTypeMap.put(type.value, type);
	    }
	}

	public static QoSNegotiationEnum fromInt(Integer value) 
	{
		QoSNegotiationEnum type = intToTypeMap.get(value);
	    return type;
	}
	
	private int value;
	
	private QoSNegotiationEnum(int value)
	{
		this.value=value;
	}
